...
void Game::initializeRace() {
	raceEntities
		.clear(); // clear all players to ensure we start from a clean slate
	initializeTrack();
...
void Game::initializeTrack() {
	...
	// create track as a static mesh with baked physics
	{
		...
		gameState.coordinator->addComponent(trackE,
											CollisionData{GROUND, trackE});
		auto &trackPhys =
			gameState.coordinator->getComponent<CollisionData>(trackE);
		auto trackActor =
		...
		gameState.coordinator->addComponent(ribbon,
											CollisionData{GROUND, ribbon});
		auto &ribbonPhys =
			gameState.coordinator->getComponent<CollisionData>(ribbon);
		...
		// Healzone Tracks
		Model healTrackModel(
			"assets/healzoneTracks.obj"); // loads model and paths
		...
		gameState.coordinator->addComponent(healTrack,
											CollisionData{GROUND, healTrack});
		auto &healTrackPhys =
			gameState.coordinator->getComponent<CollisionData>(healTrack);
		...
		auto &healPhys =
			gameState.coordinator->getComponent<CollisionData>(heal);
		...
		auto &killPhys =
			gameState.coordinator->getComponent<CollisionData>(kill);
	...
	glm::vec3 pathStartPt = trackPaths.at(0).curvePoints.at(
		0); // First point of first path (only one path for now)
	...
	if (curve.paths[0].curvePoints.size() !=
		healCurve.paths[0].curvePoints.size()) {
		dbug::log("DEF", 2,
				  "\n\n WARNING: track curves of different lengths! \n\n");
	}
	lapSys->generateCheckpoints(trackPaths);
	aiControllerSys->setStatePaths(
		trackPaths); // send track paths to the ai controller
	...
	// gameState.uiText =
	// gameState.uiSystem->raceUI(coordinator->getComponent<LapCounter>(player).currentLap);
}

void Game::initializePlayerSpark(std::vector<TrackCurve> &trackPaths,
...
void Game::initializeAISpark(std::vector<TrackCurve> &trackPaths,
							 glm::vec3 pathStartPt, std::string name) {
	PxVec3 startLoc = PxVec3(pathStartPt.x, pathStartPt.y, pathStartPt.z);
...
void Game::initializeFinishLine() {
	...
	CollisionData finishCollisionData{FINISH_LINE, -1};
	{
		...
		PxRigidStatic *triggerActor = physics->gPhysics->createRigidStatic(
...
void Game::initializeAudio() {
	music.init(gameState);

	// place holder test sounds
	// auto start = gamestate.audio->createsound("death", false);
	// auto loop = gamestate.audio->createsound("title", false);
	// music = Music(start, loop);
...
void Game::cleanupGame() {
	...
	PxU32 nbActors =
		physics->gScene->getNbActors(PxActorTypeFlag::eRIGID_STATIC);
	if (nbActors > 0) {
		std::vector<PxActor *> actors(nbActors);
		physics->gScene->getActors(PxActorTypeFlag::eRIGID_STATIC,
...
void Game::update() {
	...
	// sync menuControl back to InputSystem so controller/keyboard checks can
	...
	if (gameState.currentState == GAMEPLAY) {
		...
		if (raceCountdown.activeTimer()) {
			...
			if (secondsLeft <= 3.0 &&
				secondsLeft !=
					lastPrintedSecond) { // prevents output console spam
			...
			uiSys->updateLapCounter(
				coordinator->getComponent<LapCounter>(player).currentLap);
